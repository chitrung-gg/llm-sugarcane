    ...
    async def executor(state: PlanExecuteState) -> Command[
        ...
        try:
            ...
            if is_inner_complete:
                ...
                logger.warning(f"⚠️ [Executor] Step {current_step.step_id} failed to complete. Inner agent missing info.")
        ...
        # If the step failed, we should probably stop and ask the user, 
        # or at least route to Summarizer to explain the failure.
        if final_status == PlanStatus.FAILED:
            logger.error(f"[Executor] Step {current_step.step_id} FAILED. Breaking loop.")
