...
class FileService:
    ...
    async def get_bulk_dataset_files(self, dataset_ids: List[uuid.UUID]) -> List[UserDatasetFile]:
        """Aggregates files from both databases (userdata and genome) in bulk for given dataset IDs."""
        if not dataset_ids:
            return []
            
        all_files = []
        
        # Query 1: Fetch Knowledge Files AND their knowledge links
        async with userdata_connection_pool.connection() as conn:
            cursor = await conn.execute(
                """
                SELECT 
        ...
        # Query 2: Fetch Logical Genomes from Genome Schema
        async with genome_connection_pool.connection() as conn:
            cursor = await conn.execute(
                """
                SELECT global_id as id, dataset_id, name as file_name, 
            ...
            for row in rows:
                ...
                all_files.append(UserDatasetFile(
                    id=r_dict["id"], dataset_id=r_dict["dataset_id"],
                    file_name=r_dict["file_name"], file_type=mapped_file_type, 
                    rustfs_uri=r_dict["rustfs_uri"] or "No primary path",
                    file_metadata=r_dict["file_metadata"], created_at=r_dict["created_at"],
                    genome_global_id=r_dict["id"] 
                ))
